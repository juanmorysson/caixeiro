...
def aleatorio(grafo, vertice, aresta = None, caminho = Grafo(), invalidos = []):
    # O(1)
    ...
    if len(arestas_disponiveis) > 0:
        sorteio = randrange(len(arestas_disponiveis))
        aresta_sorterio = arestas_disponiveis[sorteio]
        if aresta_sorterio.pontoA == vertice:
            novo_vertice = aresta_sorterio.pontoB
        if aresta_sorterio.pontoB == vertice:
            novo_vertice = aresta_sorterio.pontoA
        aleatorio(grafo, novo_vertice, aresta=aresta_sorterio, caminho=caminho, invalidos=invalidos)
    else:
        c = "; Caminho: "
        ...
        if (len(caminho.vertices)<15):
            ...
            if not existe_aresta_final:
                ...
                aresta = caminho.ultima_aresta()
                caminho.remover_aresta(aresta)
                aleatorio(grafo, vertice, aresta, caminho=caminho,
                             invalidos=invalidos)
            else:
        ...
        if (len(caminho.vertices) < 15):
            ...
            if len(caminho.vertices)>=1:
                ...
                if len(caminho.arestas) > 1:
                    caminho.remover_aresta(aresta)
                aleatorio(grafo, vertice, aresta=caminho.ultima_aresta(), caminho=caminho, invalidos=invalidos)
    return caminho
